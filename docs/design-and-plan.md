...
- 展示结构（从上到下）：
  ...
  4. 页面底部邀请卡片：
     - 显示：
       ...
       - 累计邀请人数 / 今日新增邀请。
       - 专属推广链接，复制按钮。
       - 规则说明：每成功推荐 5 人，获得 +1 次额外机会，每天最多 +5 次等。
...
- 每人每天基础测算次数 `base_quota = 3`。
- 额外次数：
  - 统计该用户当天“成功邀请注册”的人数 `success_invites_today`；
  - `extra_quota_today = min(floor(success_invites_today / 5), 5)`；
  - 即每成功邀请 5 人 +1 次额外机会，每天额外最多 +5 次。
- 剩余次数：
...
- 验证：
  - 使用两个账号交叉测试邀请：
    - A 获取专属链接，B 使用该链接注册；
    - 检查 A 的邀请统计和配额变化；
    - 测试“每成功推荐 5 人 +1 次额外机会（每天最多 +5 次）”规则。
